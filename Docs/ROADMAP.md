...
- la sauvegarde/charge ForgeHistory restaure carte et ville au même tick et à la
  ...
  incompatibles ;
- le portage conserve un mapping versionné de GUID source→cible et les licences,
  ne référence aucun prefab Vendor direct et résout les écarts de
  packages/rendu sans régression de la carte principale ;
...
| `M3` | Intégration ForgeHistory et première ville | BLOCKED | Contrats, packages, rendu, transition et port d'assets sont prouvés ; adaptateur autoritaire et ville réelle attendent les décisions Hermes. |
...
|---|---|---|---|---|
| `M3-FH-01` | Contrat d'autorité, de chargement et de portage | DONE | Épingler le commit ForgeHistory audité ; cartographier carte, sélection de ville, scène, packages, rendu, données, horloge, commandes, sauvegarde et LOD ; définir `CityLaunchContext`, snapshot, intention/accusé, révision, erreurs et propriétaire de chaque donnée ; consigner les changements amont comme demandes Hermes sans écrire dans ForgeHistory. | Contrat C# pur v1, schéma JSON 2020-12 et cinq exemples cohérents ; matrice d'autorité et cycle de chargement documentés ; 8/8 tests Python et validateur hors Unity verts, `upstream_writes=0`. |
...
| 02 | PROUVÉ | `M3-FH-01` | Audit lecture seule ForgeHistory et contrat d'autorité/chargement/portage épinglé au commit observé. | Contrat C# pur, schéma JSON v1, cinq exemples, matrice exhaustive et 8/8 tests hors Unity verts. |
...
| 2026-08-13 | `M3-FH-02` | Frontière package fermée : contrats purs et lifecycle de présentation Unity sont deux packages portables ; la présentation est créée/détruite explicitement par l'hôte et refuse une double instance. Le bundle historique, ses fixtures, sa simulation, son horloge et sa sauvegarde sont réservés au laboratoire avec les assets sous `Assets/CityLabHost`. Aucun bootstrap global ni écriture ForgeHistory. | Hôte Unity minimal sans scène/fixture/laboratoire 3/3 ; 9/9 tests Python et validateur `presentation_host_tests=3 upstream_writes=0` ; CityLab 89/89 EditMode, 1/1 PlayMode ; build 308 862 268 octets et smoke GPU 20 foyers/30 bâtiments/30 habitants avec round-trip save. | `M3-FH-03` : prototyper la convergence Built-in/URP et figer la matrice de packages avant les vues/assets intégrés. |
